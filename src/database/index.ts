...
let initializing: Promise<Kysely<DatabaseSchema>> | null = null;

/**
 * 清理数据库中已有的重复数据
 * 保留最新的记录，删除其他重复项
 */
const deduplicateExistingData = async (dbInstance: Kysely<DatabaseSchema>) => {
  try {
    // 查找重复的 (type, value) 组合
...
export const getDatabase = async () => {
  ...
  initializing = (async () => {
    ...
    try {
      await sql`ALTER TABLE history ADD COLUMN fileSize integer`.execute(
        dbInstance,
      );
    } catch (_error) {}
    ...
    // 清理已有重复数据（在表结构创建完成后）
    await deduplicateExistingData(dbInstance);
